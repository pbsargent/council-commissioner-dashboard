...
function serviceAreaForDistrict(district) {
  return (state.dashboard?.dashboard?.districts || []).find((row) => row.district === district) || {};
}

function serviceAreaGroups(rows, labelKey = "label") {
  const source = state.dashboard?.dashboard?.service_areas || [];
  const sourceByName = new Map(source.map((area, index) => [area.service_area, { ...area, index }]));
  const groups = new Map();
  for (const row of rows) {
    const districtInfo = serviceAreaForDistrict(row[labelKey] || row.district);
    const name = districtInfo.service_area || "Unassigned";
    if (!groups.has(name)) {
      const sourceRow = sourceByName.get(name) || {};
      groups.set(name, {
        name,
        fieldDirector: sourceRow.field_director || districtInfo.service_area_field_director || "",
        order: sourceRow.service_area_order ?? districtInfo.service_area_order ?? sourceRow.index ?? 99,
        rows: [],
      });
    }
...
function officialDistrictSet() {
  return new Set((state.dashboard?.dashboard?.districts || []).map((row) => row.district).filter(Boolean));
}

function officialDistrictCount(rows) {
  const official = officialDistrictSet();
  const labels = new Set();
  for (const row of rows) {
    for (const label of districtLabels(rowDistrict(row))) {
      if (official.has(label)) labels.add(label);
    }
  }
  return labels.size;
}

function officialDistrictRows(rows) {
  const official = officialDistrictSet();
...
function renderKpis() {
  ...
  const tiles = [
    ...
    ["Districts", n(officialDistrictCount(rows)), "Official districts represented", "good"],
...
function barRow(row, total, max, tone = "") {
  return `
    <div class="bar-row">
...
function renderGroupedBars(targetId, rows, total, tone = "", options = {}) {
  const services = serviceAreaGroups(rows.slice(0, 14));
  const serviceCounts = services.map((service) => service.rows.reduce((sum, row) => sum + (row.count || 0), 0));
  const max = Math.max(1, ...rows.map((row) => row.count), ...serviceCounts);
  const forceOpen = Boolean(options.defaultOpen || document.getElementById("districtSelect").value || document.getElementById("searchInput").value.trim());
  document.getElementById(targetId).innerHTML = services.map((service) => {
    const count = service.rows.reduce((sum, row) => sum + (row.count || 0), 0);
    const open = forceOpen || state.openServiceAreas.has(service.name);
    const serviceRow = `
    <div class="bar-row service-bar-row">
      <div class="bar-label"><button class="service-toggle" type="button" data-service-area="${esc(service.name)}"><span class="disclosure">${open ? "-" : "+"}</span><strong>${esc(service.name)}</strong></button><span>${n(service.rows.length)} districts · ${esc(service.fieldDirector || "No field director")}</span></div>
      <div class="meter"><div class="meter-fill ${tone}" style="width:${Math.max(2, (count / max) * 100)}%"></div></div>
      <div class="bar-value">${n(count)}</div>
    </div>`;
    const detailRows = open ? service.rows.map((row) => barRow(row, total, max, tone)).join("") : "";
    return serviceRow + detailRows;
  }).join("") || '<div class="empty-state">No matching rows.</div>';
}

function renderFlatBars(targetId, rows, total, tone = "") {
  const max = Math.max(1, ...rows.map((row) => row.count));
  document.getElementById(targetId).innerHTML = rows.slice(0, 14).map((row) => barRow(row, total, max, tone)).join("")
    || '<div class="empty-state">No matching rows.</div>';
}

function renderDistributions() {
  const rows = currentRows();
  const districtRows = officialDistrictRows(rows);
  const statusRows = countBy(rows, rowStatus);
  document.getElementById("districtCount").textContent = `${n(districtRows.length)} districts`;
  renderGroupedBars("districtBars", districtRows, districtRows.reduce((total, row) => total + row.count, 0), "", { defaultOpen: true });
  renderFlatBars("stageBars", statusRows, rows.length, "good");
}
...
function bindEvents() {
  ...
  ["districtBars", "tayRows"].forEach((id) => {
...
async function init() {
  const [mondayResponse, dashboardResponse] = await Promise.all([
    fetch("https://pbsargent.github.io/council-dashboard-summary/data/monday-latest.json", { cache: "no-store" }),
    fetch("https://pbsargent.github.io/council-dashboard-summary/data/latest.json", { cache: "no-store" }),
  ]);
...
init().catch((error) => {
  document.querySelector("main").innerHTML = `
    <section class="panel">
      ...
      <p class="subtle">Run this page from a local web server or static host so it can read the Council Summary monday latest data.</p>
    </section>
  `;
});
