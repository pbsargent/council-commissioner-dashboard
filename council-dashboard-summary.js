...
function renderMondayPanel() {
  ...
  if (!monday) {
    document.getElementById("mondaySummary").innerHTML = `
      <article class="monday-stat"><strong>Monday data unavailable</strong><span>Refresh the Council Summary monday-latest data to populate this panel.</span></article>
    `;
    return;
  }
...
function renderSignals() {
  ...
  const unitCommissioners = c.unit_commissioners ?? data.dashboard.commissioners.filter((row) => /unit commissioner/i.test(row.position || "")).length;
  ...
  const cards = [
    ...
    [`${n(unitCommissioners)} UC-only commissioners`, `${n(c.units)} units and ${n(data.dashboard.priority_units.length)} priority units in the commissioner work queue.`],
...
function renderCoverage() {
  ...
  const unitCommissioners = c.unit_commissioners ?? data.dashboard.commissioners.filter((row) => /unit commissioner/i.test(row.position || "")).length;
  const rosterRecords = c.commissioner_records ?? data.dashboard.commissioners.length;
  const duplicateRecords = c.duplicate_commissioner_records ?? Math.max(0, rosterRecords - (c.commissioners || 0));
  const cards = [
    ["Unique commissioners", n(c.commissioners), `${n(rosterRecords)} roster records; ${n(duplicateRecords)} duplicate role registrations.`],
    ["UC-only commissioners", n(unitCommissioners), `${n(c.units)} council units; ratios exclude ADC/DC dual registrations.`],
...
async function init() {
  ...
  try {
    const mondayResponse = await fetch("https://pbsargent.github.io/council-dashboard-summary/data/monday-latest.json", { cache: "no-store" });
